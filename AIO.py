...
class ConsumptionModel:
    ...
    @torch.no_grad()  # Add this decorator to disable gradient computation
    def plot_decision_rule(self):
        ...
        w_numpy = w_vec.cpu().detach().numpy()
        zeta_numpy = (w_vec * zeta_vec).cpu().detach().numpy()
        
        plt.figure(figsize=(10, 6))
        plt.title("Multidimensional Consumption-Savings (decision rule)")
        plt.plot(w_numpy, w_numpy, linestyle='--', color='black', label='45° line')
        plt.plot(w_numpy, zeta_numpy, label='Consumption')
        plt.xlabel("$w_t$")
        plt.ylabel("$c_t$")
        plt.grid(True)
        plt.legend()
    ...
    def train(self, num_epochs: int, batch_size: int = 1000) -> List[float]:
        ...
        self.model.train()  # Add explicit train mode
        for _ in pbar:
        ...
        self.model.eval()  # Set to eval mode after training
        return losses
...
def main():
    ...
    plt.figure(figsize=(10, 6))
    losses_np = np.array(losses)  # Convert list to numpy array
    plt.plot(np.sqrt(losses_np))
    plt.xscale('log')
    plt.yscale('log')
    plt.grid(True)
